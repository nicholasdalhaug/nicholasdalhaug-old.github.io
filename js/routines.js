const routinesDOM = document.querySelector('.cf-routines');
const preloaderDOM = document.querySelector('.cf-preloaders');

// load routines
function displayRoutine(id, data) {
  var routine_html = `
    <div class="card-panel cf-routines-panels" data-id="${id}">
      <div class="cf-routines-names truncate">${data.name}</div>
      <div class="cf-routines-settings">
    `;
  return data.exercises.reduce((promiseChain, routineExerciseId) => {
    return promiseChain.then(() => new Promise((resolve) => {
      routineExerciseId.get().then( exercise => {
        routine_html += `
          <div class="truncate">${exercise.data().name}</div>
        `;
        resolve();
      });
    }));
    ...
    routinesDOM.innerHTML += routine_html;
  });
}

dbGetAll('routines', displayRoutine)
.then( () => {
